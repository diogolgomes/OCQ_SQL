SELECT 
   ...
   CASE
        WHEN BUSINESS_UNIT='REVENDA' OR BUSINESS_UNIT='BREU E DERIVADOS' THEN to_char(trim(TECNOLOGIA))      
    END AS SUBCLASSE,
    SUM(QTD) AS QTD,
    ...
    ,NULL AS SUB
    ,'' as conta_contabil
    ,nomedaculuna as TESTE
FROM PUB_JDE_SALES  
left join proddta.f0006 on trim(MCMCU)=triM(UN_NEG)
LEFT JOIN (
    SELECT 
        DISTINCT  
        CICO AS cia,
    ...
    AND TRIM(IMSRP5) IN ('PA','MP')
    AND IMLITM NOT LIKE '%.%'
) ON to_char(MCAN8)=to_char(cia) AND SEGMENT=SEGMENTO 
WHERE  INTERCOMPANY IS NULL AND TRIM(CLIENTE) NOT IN ('REVOCHEMICAL IND E COM E PROD QUIM LTDA','ELEKEIROZ SA','OSWALDO CRUZ QUIMICA CHILE SPA' ) AND filial NOT IN ('191000') AND tipo_pedido NOT IN ('VI','VU')
GROUP BY 
...
SELECT distinct
    ...
    CASE WHEN srp1.DRDL01 = 'REVENDA' or srp1.DRDL01 = 'BREU E DERIVADOS' THEN to_char(TRIM(srp2.DRDL01))
    END AS SUBCLASSE,
    0 AS QTD, 
...
RIGHT JOIN PUBDB.DESPDEL D ON TRIM(A.CIMCU) = CENTRO and D.KCO=A.CICO
where srp1.DRDL01 is not null
group by A.CIMCU, srp1.DRDL01,srp2.DRDL01,D.CENTRO,TO_CHAR(D.MES),TO_CHAR(D.ANO),D.OBJ,D.SUB