...
AND IBSTKT NOT IN ('O')
AND */suBSTR( TRIM(a.ixmMcu),1,1) IN ('1','3','4','6')
...
--decode(instr(trim(IMLITM),'.'),'0',trim(IMLITM),substr(trim(IMLITM),'1',instr(trim(IMLITM),'.')-1)) AS ITEM, 
    ...
    WHERE (TRLOTN)='MODELO                        ' AND (TRQTST) IN ('SOLIDOS                  ') AND TRMCU='      111000'
      ...
      AND IMSRP1 NOT IN ('160','170','180','360','200','493') AND IMSRP1||IMSRP2 NOT IN ('470111','470261','492285')
     AND IMSTKT NOT IN ('O')
...
UNION ALL
    ...
    LEFT OUTER JOIN PRODDTA.F4101 ON TRITM = IMITM
    WHERE  (TRLOTN)='MODELO                        ' AND (TRQTST) IN ('SOLIDOS3                 ') AND TRMCU='      111000'
      AND IMSRP1  IN ('470','140','492') and imsrp2 IN ('261','279','280','281','283','285')
      AND IMSTKT NOT IN ('O')
            ...
            WHEN CLASSE_MERCADORIA='360' AND SUB_CLASSE_MERCADORIA in ('220','136') AND SOMA_TOTAL<2 AND SOMA_TOTAL>0.8 THEN 1
             ...
             WHEN CLASSE_MERCADORIA IN ('170','160','180','360','200','493')  AND SOMA_TOTAL<14200 AND SOMA_TOTAL>13800 THEN  14000
                WHEN CLASSE_MERCADORIA IN ('170','160','180','360','200','493')  AND SOMA_TOTAL<1850 AND SOMA_TOTAL>1750 THEN  1800
			             ...
			             when QTD_LISTA>0  and CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285') then QTD_LISTA
               WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')  AND SOMA_TOTAL<1.30 AND SOMA_TOTAL>0.8 THEN  1
             WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')    AND SOMA_TOTAL<1200 AND SOMA_TOTAL>900 THEN  1000
			        WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')  AND SOMA_TOTAL<9300 AND SOMA_TOTAL>8700 THEN  9000
					       WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')    AND SOMA_TOTAL<8300 AND SOMA_TOTAL>7700 THEN  8000
						          WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')    AND SOMA_TOTAL<120 AND SOMA_TOTAL>90 THEN  100
             WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')   AND SOMA_TOTAL<14200 AND SOMA_TOTAL>13800 THEN  14000
                WHEN CLASSE_MERCADORIA IN ('470','140','492')  AND SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285')   AND SOMA_TOTAL<1850 AND SOMA_TOTAL>1750 THEN  1800
...
FROM  PRODDTA.F4101 
   ...
   WHERE IMPRP1  IN ('470','140','492')  AND IMPRP2 in ('261','279','280','281','283','285')
    AND  /* decode(instr(trim(IMLITM),'.'),'0',trim(IMLITM),substr(trim(IMLITM),'1',instr(trim(IMLITM),'.')-1))*/ trim(IMLITM) NOT IN  (SELECT TRIM(ITEM) FROM PRODUTOS_SOLIDOS)
    AND IMSTKT NOT IN ('O')
...
A.ITEM_PAI, 
        ...
        CASE
		  ...
		  WHEN TRIM(A.CLASSE_MERCADORIA) IN ('470','492')  AND TRIM(A.SUB_CLASSE_MERCADORIA) in ('261','279','280','281','283','285') THEN A.SOMA
...
A.ITEM_PAI,
		...
		WHEN  A.CLASSE_MERCADORIA IN ('470','492') AND A.SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285') THEN A.QTD/A.SOMA
       ELSE A.QTD/(A.SOLIDO_TEORICO/ A.SOLIDO_PRODUTO)/100 END AS QTD,
		...
		WHEN  A.CLASSE_MERCADORIA IN ('470','492') AND A.SUB_CLASSE_MERCADORIA in ('261','279','280','281','283','285') THEN A.QTD/A.SOMA * A.CUSTO_TOTAL
...
AND suBSTR( TRIM(ixmMcu),1,1) IN ('1','3','4','6')
...
FILIAL,
SYSDATE, 
          TO_CHAR(ITEM_PAI) ITEM_PAI, 
          trim(imdsc1) as DSC,  
          TO_CHAR(LISTA) LISTA,
          ROUND(SUM(CUSTO_UNITARIO),6) AS CUSTO_UNITARIO,
                                 ROUND(SUM(CUSTO_UNITARIO),6)/(select trim(a.cxcrrd)  from proddta.F0015 a   where a.cxcrcd = 'BRL' and a.cxcrdc ='USD' and a.cxeft = (select max(b.cxeft) from proddta.F0015 b  where a.cxcrcd = b.cxcrcd and a.cxcrdc =b.cxcrdc)) CUSTO_DOLAR,

          ROUND(CUSTO_SERV,6) CUSTO_SERV, 
		     ROUND(CUSTO_MATE,6) CUSTO_MATE, 
    substr(DESCRICAO_LISTA,2,200) DESCRICAO_LISTA, 
         (select trim(a.cxcrrd)  from proddta.F0015 a   where a.cxcrcd = 'BRL' and a.cxcrdc ='USD' and a.cxeft = (select max(b.cxeft) from proddta.F0015 b  where a.cxcrcd = b.cxcrcd and a.cxcrdc =b.cxcrdc)) as BRL_USD,
        (select trim(a.cxcrrd)  from proddta.F0015 a   where a.cxcrcd = 'BRL' and a.cxcrdc ='EUR' and a.cxeft = (select max(b.cxeft) from proddta.F0015 b  where a.cxcrcd = b.cxcrcd and a.cxcrdc =b.cxcrdc)) as BRL_EUR
,ROUND(SUM(CUSTO_UNITARIO),6)+ ROUND(CUSTO_SERV,6)+ ROUND(CUSTO_MATE,6) AS CUSTO_TOTAL
...
GROUP BY TO_CHAR(ITEM_PAI),trim(imdsc1),TO_CHAR(LISTA),ROUND(CUSTO_SERV,6) ,ROUND(CUSTO_MATE,6),DESCRICAO_LISTA,FILIAL,CLASSE,TRIM(DRDL01)
ORDER BY DSC,LISTA